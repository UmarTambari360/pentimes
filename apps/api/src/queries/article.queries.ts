...
import { 
    ...
    categories, 
    users, 
    likes, bookmarks, comments }                     from '../db/schema/index.js';
...
export async function findArticlesByAuthor(authorId: string, limit = 12, offset = 0) {
...
export async function getArticleCounts(articleId: string) {
  const [likeResult, commentResult] = await Promise.all([
    db.select({ count: count() }).from(likes).where(eq(likes.articleId, articleId)),
...
export async function searchArticles(query: string, limit = 12, offset = 0) {
  const tsQuery = query.trim().split(/\s+/).join(' & ');

  const items = await db.query.articles.findMany({
    where: and(
      eq(articles.status, 'published'),
      sql`to_tsvector('english', coalesce(${articles.title}, '') || ' ' || coalesce(${articles.excerpt}, '') || ' ' || coalesce(${articles.content}, '')) @@ to_tsquery('english', ${tsQuery})`
    ),
